			...
			{:else}
				...
				<article class="space-y-8">
							...
							{#if isAuthenticated}
								<div class="flex items-center gap-2">
									<Button
										variant="outline"
										size="sm"
										class="h-9 gap-2 px-4 text-xs font-semibold"
					...
					{#if page.data.currentUser && flashcardsQuery.data && flashcardsQuery.data.length > 0}
						<div
							class="flex items-center justify-between rounded-xl border border-primary/20 bg-gradient-to-br from-primary/5 to-primary/10 p-5"
						>
							<div class="flex items-center gap-4">
								<div
									class="flex h-10 w-10 items-center justify-center rounded-lg bg-background text-primary shadow-sm"
								>
									<Brain class="h-5 w-5" />
								</div>
								<div>
									<p class="font-semibold text-foreground">
										{flashcardsQuery.data.length} Flashcards Available
									</p>
									<p class="text-xs text-muted-foreground">Test your knowledge on this topic</p>
								</div>
							</div>
					...
					{#if item.subject}
						<footer
							class="flex items-center justify-between rounded-xl border bg-muted/20 px-5 py-4"
						>

...
<main class="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6">
	...
	<p class="mt-1 text-sm text-muted-foreground">
		Manage your account settings for the social pivot.
	</p>
	...
	{:else}
		<div class="mx-auto mt-5 max-w-3xl rounded-lg border border-border bg-card p-4">
			...
			<p class="text-sm">
				...
				{meQuery.data.username ?? 'not set'}
			</p>
			<div class="mt-4 flex gap-2">
				<Button href="/settings/username"
					>{meQuery.data.username ? 'View Username' : 'Set Username'}</Button
				>
				<Button
					variant="outline"
					href="/u/{meQuery.data.username ?? 'unknown'}"
					disabled={!meQuery.data.username}>Open Profile</Button
				>
			</div>
		</div>
	{/if}
	...
	{#if profileQuery.data?.bio}
		<div
			class="mx-auto mt-4 max-w-3xl rounded-lg border border-border bg-card p-4 text-sm text-muted-foreground"
		>

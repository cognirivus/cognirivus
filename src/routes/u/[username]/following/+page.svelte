<script lang="ts">
	...
	const followingQuery = useQuery((api as any).social_graph.listFollowingUsers, () => ({
		followerAuthId: profileQuery.data?.authId ?? '',
		limit: listLimit
	}));
	const followingUsers = $derived((followingQuery.data ?? []) as Array<FollowUserListItem>);
	const showMoreForGuests = $derived(
		!auth.isAuthenticated && (profileQuery.data?.followingCount ?? 0) > 10
	);
</script>
			...
			<div class="mb-4 flex flex-wrap items-center justify-between gap-2">
				<div>
					...
					<p class="text-sm text-muted-foreground">
						u/{profileQuery.data.username} follows {profileQuery.data.followingCount} users
					</p>
				</div>
				<div class="flex gap-2">
					<Button variant="outline" href="/u/{username}/followers">Followers</Button>
				</div>
			</div>

			{#if followingQuery.isLoading && profileQuery.data.followingCount > 0}
				<p class="text-sm text-muted-foreground">Loading following...</p>
			{:else if followingUsers.length === 0}
				<Card class="gap-0 py-4">
					<CardContent class="text-sm text-muted-foreground">Not following anyone yet.</CardContent>
				</Card>
			{:else}
				<ul class="space-y-2">
					{#each followingUsers as user (user.authId)}
						<li>

...
export const extractFromNews = action({
	...
	handler: async (ctx: any, args: { limit?: number }): Promise<string> => {
		...
		for (const newsItem of newsItems) {
			...
			const processed = await ctx.runQuery(internal.content.isNewsProcessed as any, { newsId });
			if (processed) {
				console.log(`News item ${newsId} (${newsItem.date}) already processed. Skipping.`);
				continue;
			}

			console.log(`Processing: ${newsItem.date}...`);
...
5. TONE: Maintain a professional, educational, and factual tone.`
				...
				for (const loc of extractedLocations) {
					if (loc.title && loc.text) {
						await ctx.runMutation(internal.content.saveExtractedLocation as any, {
							title: loc.title,
							text: loc.text,

...
get-convex/aggregate\\
\\
View repo](https://github.com/get-convex/aggregate) [![GitHub logo](https://www.convex.dev/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Fnpm.bd053a67.svg&w=48&q=75)View package](https://www.npmjs.com/package/@convex-dev/aggregate)

### Category

[Database](https://www.convex.dev/components#database 'Show all components in "Database" category')

![Aggregate hero image](https://www.convex.dev/_next/image?url=%2F_next%2Fstatic%2Fmedia%2Faggregate.b12b0a6f.png&w=1536&q=75)
...
## Examples [\#](https://www.convex.dev/components/aggregate#examples)

## What are Aggregates for? [\#](https://www.convex.dev/components/aggregate#what-are-aggregates-for)
...
4. Nothing, use `key=null` for everything if you just want [a total count, such as for random access](https://www.convex.dev/components/aggregate#total-count-and-randomization).

### Grouping [\#](https://www.convex.dev/components/aggregate#grouping)
...
instances. See [below](https://www.convex.dev/components/aggregate#defining-multiple-aggregates) for details.
...
[below](https://www.convex.dev/components/aggregate#read-dependencies-and-writes) for more details. To avoid interference,
you can use Namespaces.

### Namespacing [\#](https://www.convex.dev/components/aggregate#namespacing)
...
[example/convex/photos.ts](https://github.com/get-convex/aggregate/blob/main/example/convex/photos.ts).

### More examples [\#](https://www.convex.dev/components/aggregate#more-examples)
...
- [Offset-based pagination](https://www.convex.dev/components/aggregate#offset-based-pagination): view the 14th page of photos, where each page has 50 photos.
- [Random access](https://www.convex.dev/components/aggregate#total-count-and-randomization): Look up a random song in a playlist, as the next song to play.

Try it out: [https://aggregate-component-example.netlify.app/](https://aggregate-component-example.netlify.app/)

## Pre-requisite: Convex [\#](https://www.convex.dev/components/aggregate#pre-requisite-convex)
...
## Installation [\#](https://www.convex.dev/components/aggregate#installation)

See [`example/`](https://github.com/get-convex/aggregate/blob/main/example/convex/) for a working demo.
...
2. Create a [`convex.config.ts`](https://github.com/get-convex/aggregate/blob/main/example/convex/convex.config.ts) file in your
...
### Defining multiple aggregates [\#](https://www.convex.dev/components/aggregate#defining-multiple-aggregates)
...
## Usage [\#](https://www.convex.dev/components/aggregate#usage)

### Write to the aggregate data structure [\#](https://www.convex.dev/components/aggregate#write-to-the-aggregate-data-structure)

Usually you want to aggregate data in a Convex table. If you're aggregating data
that's not in a table, you can use the
[lower-level API](https://www.convex.dev/components/aggregate#aggregate-without-a-table).
...
Pick your key as described [above](https://www.convex.dev/components/aggregate#what-are-aggregates-for). For example,
...
[tips](https://www.convex.dev/components/aggregate#automatically-update-aggregate-when-table-changes) below.
...
[run a migration to backfill](https://www.convex.dev/components/aggregate#attach-aggregate-to-an-existing-table).

### Calculate aggregates [\#](https://www.convex.dev/components/aggregate#calculate-aggregates)
...
[`example/convex/leaderboard.ts`](https://github.com/get-convex/aggregate/blob/main/example/convex/leaderboard.ts), and see the
docstrings on [the Aggregate class](https://github.com/get-convex/aggregate/blob/main/src/client/index.ts).

## Example use-cases [\#](https://www.convex.dev/components/aggregate#example-use-cases)
...
### Total Count and Randomization [\#](https://www.convex.dev/components/aggregate#total-count-and-randomization)
...
See more examples in [`example/convex/shuffle.ts`](https://github.com/get-convex/aggregate/blob/main/example/convex/shuffle.ts),
including a paginated random shuffle of some music.

### Offset-based pagination [\#](https://www.convex.dev/components/aggregate#offset-based-pagination)
...
See the full example in [`example/convex/photos.ts`](https://github.com/get-convex/aggregate/blob/main/example/convex/photos.ts).

### Aggregate without a table [\#](https://www.convex.dev/components/aggregate#aggregate-without-a-table)
...
See [`example/convex/stats.ts`](https://github.com/get-convex/aggregate/blob/main/example/convex/stats.ts) for an example.

## Operations [\#](https://www.convex.dev/components/aggregate#operations)
...
### Attach Aggregate to an existing table [\#](https://www.convex.dev/components/aggregate#attach-aggregate-to-an-existing-table)
...
3. Use a paginated background [migration](https://www.npmjs.com/package/@convex-dev/migrations) to walk all existing data and call `insertIfDoesNotExist`. In the example, you would run `runAggregateBackfill` in [leaderboard.ts](https://github.com/get-convex/aggregate/blob/main/example/convex/leaderboard.ts).
...
### Automatically update aggregate when table changes [\#](https://www.convex.dev/components/aggregate#automatically-update-aggregate-when-table-changes)

It's important that _every_ modification to the table also updates the
associated aggregate. If they get out of sync then computed aggregates might be
incorrect. Then you might have to [fix them](https://www.convex.dev/components/aggregate#repair-incorrect-aggregates).
...
The [`example/convex/photos.ts`](https://github.com/get-convex/aggregate/blob/main/example/convex/photos.ts) example uses a
trigger.

### Repair incorrect aggregates [\#](https://www.convex.dev/components/aggregate#repair-incorrect-aggregates)
...
follow the instructions from [above](https://www.convex.dev/components/aggregate#attach-aggregate-to-an-existing-table).
...
## Performance Optimizations [\#](https://www.convex.dev/components/aggregate#performance-optimizations)

### Batch Operations [\#](https://www.convex.dev/components/aggregate#batch-operations)
...
## Reactivity and Atomicity [\#](https://www.convex.dev/components/aggregate#reactivity-and-atomicity)
...
### Reactivity [\#](https://www.convex.dev/components/aggregate#reactivity)
...
### Transactions [\#](https://www.convex.dev/components/aggregate#transactions)
...
### Read dependencies and writes [\#](https://www.convex.dev/components/aggregate#read-dependencies-and-writes)
...
### Put bounds on aggregated data [\#](https://www.convex.dev/components/aggregate#put-bounds-on-aggregated-data)
...
### Lazy aggregation [\#](https://www.convex.dev/components/aggregate#lazy-aggregation)
